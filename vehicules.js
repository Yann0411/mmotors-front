...
async function rechercherVehicules(depuisBouton = false) {
  ...
  if (depuisBouton) {
    ...
    if (!km && !prixMin && !prixMax) {
      msgRecherche.style.color = "red";
      msgRecherche.textContent =
        "Veuillez remplir au moins un filtre (prix min, prix max ou kilométrage).";
      return;
    }
    msgRecherche.style.color = "";
  }

  const aucunFiltre =
    document.getElementById("filtre-marque").selectedIndex === 0 &&
  ...
  try {
    ...
    vehicules.forEach(function (vehicule) {
      // label du bouton selon le type
      const labelBtn =
        vehicule.typeOffre === "ACHAT"
          ? "Déposer un dossier d'achat"
          : vehicule.typeOffre === "LOCATION_ACHAT"
            ? "Déposer un dossier LOA"
            : "Déposer un dossier de location";
      ...
      const labelPrix =
        vehicule.typeOffre === "ACHAT" ? "Prix" : "Prix mensuel";
      prix.textContent =
        labelPrix + " : " + vehicule.prix.toLocaleString("fr-FR") + " €";
...
document
  ...
  .addEventListener("change", function () {
    ...
    selectModele.innerHTML =
      '<option value="">Tous les modèles</option>' +
      modelesUniques.map((m) => `<option value="${m}">${m}</option>`).join("");

    rechercherVehicules();
  });
document
  .getElementById("filtre-modele")
  .addEventListener("change", rechercherVehicules);
// document.getElementById('filtre-modele').addEventListener('input', rechercherVehicules);
document
  .getElementById("filtre-prix-min")
  .addEventListener("input", rechercherVehicules);
document
  .getElementById("filtre-prix-max")
  .addEventListener("input", rechercherVehicules);
document
  .getElementById("filtre-km")
  .addEventListener("input", rechercherVehicules);
