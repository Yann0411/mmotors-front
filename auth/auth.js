...
function afficherErreur(idErreur, message) {
  ...
  const input =
    wrapper && wrapper.classList.contains("champ-mdp")
      ? wrapper.querySelector("input")
      : el.previousElementSibling;

  if (input && input.tagName === "INPUT")
    input.classList.toggle("input-erreur", !!message);
}
...
async function reinitialiserMotDePasse() {
  ...
  try {
    await axios.post(
      "https://mmotors-back-production.up.railway.app/auth/reinitialiser-mot-de-passe",
...
async function sInscrire() {
  ...
  try {
    await axios.post(
      "https://mmotors-back-production.up.railway.app/auth/inscription",
    ...
    if (Array.isArray(data)) {
      ...
      typeof data === "string" &&
      data.toLowerCase().includes("déjà utilisé")
    ) {
      afficherErreur(
        "err-inscription-email",
        "Cette adresse email est déjà associée à un compte. Essayez de vous connecter.",
      );
    } else {
      msg.style.color = "red";
      msg.textContent = data || "Une erreur est survenue. Veuillez réessayer.";
...
async function seConnecter() {
  ...
  try {
    const response = await axios.post(
      "https://mmotors-back-production.up.railway.app/auth/connexion",
      {
        email,
        motDePasse,
      },
