...
async function chargerVehicules() {
  try {
    const response = await axios.get(
      "https://mmotors-back-production.up.railway.app/vehicules",
    );
    ...
    document.getElementById("list-marques-admin").innerHTML = marques
    ...
    vehicules.forEach(function (v) {
      ...
      infos.textContent =
        ...
        " € | Km : " +
        v.kilometrage.toLocaleString("fr-FR") +
        " | " +
        (typeOffreLabels[v.typeOffre] || v.typeOffre);
    ...
    document.getElementById("liste-vehicules-admin").innerHTML =
      '<p style="color:red;">Impossible de charger les véhicules.</p>';
  }
}
...
function remplirFormulaire(id, marque, modele, annee, prix, km, typeOffre) {
  ...
  document.getElementById("titre-form-vehicule").textContent =
    "Modifier un véhicule";
  // efface les erreurs visuelles
  document
    .querySelectorAll(".admin-formulaire .input-erreur")
    .forEach((el) => el.classList.remove("input-erreur"));
  document.getElementById("msg-vehicule").textContent = "";
  document
    .querySelector(".admin-formulaire")
...
async function sauvegarderVehicule() {
  ...
  try {
    if (id) {
      await axios.put(
        "https://mmotors-back-production.up.railway.app/admin/vehicules/" + id,
        vehicule,
        { headers },
      );
    } else {
      await axios.post(
        "https://mmotors-back-production.up.railway.app/admin/vehicules",
        vehicule,
        { headers },
      );
    }
    ...
    msg.textContent = Array.isArray(data)
      ? data.join(" | ")
...
async function supprimerVehicule(id, marque, modele, annee) {
  if (
    !confirm(
      `Voulez-vous supprimer le véhicule ${marque} ${modele} (${annee}) ?\n\nCette action est irréversible.`,
    )
  )
    return;
  try {
...
async function chargerDossiers() {
  try {
    ...
    dossiers.forEach(function (d) {
      ...
      type.appendChild(
        document.createTextNode(typeOffreLabels[d.typeOffre] || d.typeOffre),
      );
      ...
      btnValider.addEventListener("click", function () {
        changerStatut(d.id, "VALIDE", btnValider);
      });
      ...
      btnRefuser.addEventListener("click", function () {
        changerStatut(d.id, "REFUSE", btnRefuser);
      });
      ...
      if (d.statut !== "EN_ATTENTE") {
        ...
        btnSupprimer.textContent = "Supprimer";
        btnSupprimer.addEventListener("click", function () {
          supprimerDossier(d.id, d.clientEmail);
        });
        btnGroupe.appendChild(btnSupprimer);
      }
    ...
    document.getElementById("liste-dossiers-admin").innerHTML =
      '<p style="color:red;">Impossible de charger les dossiers.</p>';
  }
}
...
async function changerStatut(id, statut, btn) {
  ...
  try {
    await axios.put(
      "https://mmotors-back-production.up.railway.app/admin/dossiers/" + id,
      { statut },
      { headers },
    );
    afficherToast(
      statut === "VALIDE" ? "✓ Dossier validé" : "✓ Dossier refusé",
      statut === "VALIDE" ? "succes" : "erreur",
    );
    chargerDossiers();
  } catch (error) {
...
async function supprimerDossier(id, clientEmail) {
  if (
    !confirm(
      `Voulez-vous supprimer le dossier de ${clientEmail} ?\n\nCette action est irréversible.`,
    )
  )
    return;
  try {
...
document
  .getElementById("vehicule-marque")
  .addEventListener("input", function () {
    ...
    const modeles = tousLesVehicules
      .filter(
        (v) =>
          !marqueChoisie ||
          v.marque.toLowerCase() === marqueChoisie.toLowerCase(),
      )
      .map((v) => v.modele);
    const modelesUniques = [...new Set(modeles)].sort();
    document.getElementById("list-modeles-admin").innerHTML = modelesUniques
      .map((m) => `<option value="${m}">`)
      .join("");
  });
