...
if (params.get("marque")) {
  ...
  document.getElementById("info-marque-modele").textContent =
    params.get("marque") + " " + params.get("modele");

  document.getElementById("info-annee").textContent = params.get("annee");

  document.getElementById("info-km").textContent =
  ...
  document.getElementById("typeOffre").value =
    params.get("typeOffre") || "ACHAT";

  // Message devient optionnel
  document.getElementById("label-message").innerHTML =
    'Message complémentaire <span style="color:#64748B; font-weight:400;">(optionnel)</span>';
  document.getElementById("message").placeholder =
    "Ajoutez un message si vous le souhaitez...";
  document.getElementById("message").value =
    "Bonjour, je suis intéressé par ce véhicule, pouvez-vous me recontacter ?";
}
...
async function deposerDossier() {
  ...
  if (params.get("marque")) {
    vehiculeInfo =
      ...
      params.get("annee") +
      " - " +
      parseInt(params.get("km")).toLocaleString("fr-FR") +
      " km" +
      " - " +
      parseFloat(params.get("prix")).toLocaleString("fr-FR") +
      " €" +
  ...
  try {
    await axios.post(
      "https://mmotors-back-production.up.railway.app/dossiers",
    ...
    setTimeout(() => window.location.href = '../espaceClient/espaceClient.html', 2000);

  } catch (error) {
    msg.style.color = "red";
    msg.textContent =
      error.response?.data || "Erreur lors de l'envoi. Veuillez réessayer.";
  } finally {
    if (succes) {
